...
class ExerciseAgent(AbstractAgent):
    ...
    async def run_sync(
        ...
        T13.5 Task 8: 包 try/except，异常时推 FAILED 事件 payload 含 tb，
        让前端 SSE 能直接看到完整 traceback（无需 grep worker 日志）。
        """
        ...
        try:
            ...
            log.error("exercise.unhandled_exception", trace_id=trace_id, error=repr(e), tb=tb_str)
            await progress_publish(trace_id, ProgressEvent(
                stage=Stage.EXERCISE, status="failed",
                payload={"code": "exercise_unhandled", "message": repr(e), "tb": tb_str},
            ))
            raise AppError(ErrorCode.INTERNAL, f"Exercise 处理失败: {e}", trace_id=trace_id) from e