...
| V4.0 | 2026-07-11 | 团队 | 按附录 A 决策修订（基于 V3.5 文本，仅改正文具体值，不动文档结构）：(1)AppId 新命名清单 (2)WindowState 加 contentState/metadata (3)画像 6 维改为中文第三套 (4)关卡形式 4 种无听力型 (5)分支状态拆分为 status + branchStatus (6)持久化键名按 § 5.2 (7)快捷键引入 scope (8)mitt 事件总线提前重构 (9)ChatFloat 改真窗口 (10)Settings 单实例。详见 `docs/superpowers/specs/2026-07-11-window-animation-and-v4-alignment-design.md` |
...
> **v4 附录 A #1 扩展说明**：在前述 10 个核心应用之外，新增两个 demo-serif 阶段引入的 AppId：
> - `task_list`（今日学习，单实例）：demo 阶段的"任务清单"窗口，与 v3 § 2.2 中描述的关卡化学习闭环中的"今日计划"对应。
> - `profile`（六维画像，可多实例）：demo 阶段在右下角常驻的画像雷达图窗口。
>
> 这两个 AppId 在生产完整版时，可考虑合并入 `dashboard` 或 `settings`。
